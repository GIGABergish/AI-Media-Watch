...
import { useLang, pick, riskLabel, statusLabel } from '../i18n';
import { DemoCase, RiskLevel, Platform, CaseStatus } from '../types';
import { getRiskBg, getRiskColor, getStatusStyle, formatDate } from '../lib/utils';
...
export default function QueuePage({ onSelectCase }: QueuePageProps) {
  ...
  const queueData: DemoCase[] = [
    ...demoCases,
    {
      ...demoCases[0],
      id: 'q-001',
      title: pick(lang, 'Quick earnings on sports predictions', 'Быстрый заработок на спорт прогнозах'),
      platform: 'TikTok',
      riskScore: 79,
      riskLevel: 'high',
      status: 'new',
      uploadDate: '2026-06-22',
      mainReason: pick(lang, 'Promise of 90% prediction accuracy', 'Обещание 90% точности прогнозов'),
    },
    {
      ...demoCases[1],
      id: 'q-002',
      title: pick(lang, 'Cryptocurrency signal bot', 'Криптовалютный сигнальный бот'),
      platform: 'Telegram',
      riskScore: 83,
      riskLevel: 'high',
      status: 'reviewing',
      uploadDate: '2026-06-22',
      mainReason: pick(lang, 'Referral scheme + profit guarantee', 'Реферальная схема + гарантия прибыли'),
    },
    {
      ...demoCases[3],
      id: 'q-003',
      title: pick(lang, 'NFT club: 200% income per month', 'NFT клуб: доход 200% за месяц'),
      platform: 'Instagram',
      riskScore: 76,
      riskLevel: 'high',
      status: 'new',
      uploadDate: '2026-06-21',
      mainReason: pick(lang, 'Unrealistic NFT returns + referral', 'Нереальная доходность NFT + реферал'),
    },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder={pick(lang, 'Search by title...', 'Поиск по названию...')}
            value={search}
        ...
        >
          ...
          {pick(lang, 'Filters', 'Фильтры')}
        </button>
        <div className="ml-auto text-xs text-slate-500">
          {pick(lang, 'Shown', 'Показано')}: <span className="text-white font-mono">{filtered.length}</span> / {queueData.length}
        </div>
      </div>
      ...
      {showFilters && (
        ...
        >
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block mb-2">{pick(lang, 'Risk level', 'Уровень риска')}</label>
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block mb-2">{pick(lang, 'Status', 'Статус')}</label>
              <div className="flex flex-wrap gap-1.5">
                <button
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block mb-2">{pick(lang, 'Platform', 'Платформа')}</label>
              <div className="flex flex-wrap gap-1.5">
                <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {[
                  pick(lang, 'Video', 'Видео'),
                  pick(lang, 'Platform', 'Платформа'),
                  pick(lang, 'Risk Score', 'Risk Score'),
                  pick(lang, 'Category', 'Категория'),
                  pick(lang, 'Main reason', 'Главная причина'),
                  pick(lang, 'Status', 'Статус'),
                  pick(lang, 'Date', 'Дата'),
                  '',
                ].map((h, hi) => (
                  <th key={h || hi} className="text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider px-4 py-3 whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filtered.map((c, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <span className={`text-[10px] ${getRiskColor(c.riskLevel)}`}>{riskLabel(c.riskLevel, lang)}</span>
                  </td>
                  <td className="px-4 py-3">
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-slate-500 text-sm">
              {pick(lang, 'No records match the filters', 'Нет записей, соответствующих фильтрам')}
            </div>
          )}
        </div>
      </div>
